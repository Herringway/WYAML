...
package immutable dchar[dchar] toEscapes;

package alias escapeSeqs  = AliasSeq!(
	cast(dchar) '0',
	cast(dchar) 'a',
	cast(dchar) 'b',
	cast(dchar) 't',
...
package alias escapePairs = AliasSeq!(
...
package alias extraEscapes = AliasSeq!(
	Tuple!(dchar,dchar)('\x09', '\t'),
	Tuple!(dchar,dchar)(' ', ' ')
);

/// All YAML escapes.
package alias escapeHexSeq = AliasSeq!('x', 'u', 'U');
...
package dchar fromEscape(dchar escape) @safe pure nothrow @nogc {
	switch(escape) {
...
/// Need a function as associative arrays don't work with @nogc.
/// (And this may be even faster with a function.)
package uint escapeHexLength(dchar hexCode) @safe pure nothrow @nogc {
	switch (hexCode) {
		case 'x': return 2;
		case 'u': return 4;
		case 'U': return 8;
