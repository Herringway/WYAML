...
version (unittest) {
	...
	Node[] constructBinary() {
		...
		return [Node([pair("canonical", canonical), pair("generic", generic), pair("description", description)])];
	}

	Node[] constructBool() {
		return [Node([pair("canonical", true), pair("answer", false), pair("logical", true), pair("option", true), pair("but", [pair("y", "is a string"), pair("n", "is a string")])])];
	}
	...
	Node[] constructInt() {
		return [Node([pair("canonical", 685230L), pair("decimal", 685230L), pair("octal", 685230L), pair("hexadecimal", 685230L), pair("binary", 685230L), pair("sexagesimal", 685230L)])];
	}
	...
	Node[] constructSet() {
		return [Node([pair("baseball players", [Node("Mark McGwire"), Node("Sammy Sosa"), Node("Ken Griffey")]), pair("baseball teams", [Node("Boston Red Sox"), Node("Detroit Tigers"), Node("New York Yankees")])])];
	}
	...
	Node[] duplicateMergeKey() {
		return [Node([pair("foo", "bar"), pair("x", 1L), pair("y", 2L), pair("z", 3L), pair("t", 4L)])];
	}

	Node[] floatRepresenterBug() {
		return [Node([pair(cast(real) 1.0, 1L), pair(real.infinity, 10L), pair(-real.infinity, -10L), pair(real.nan, 100L)])];
	}
	...
	Node[] moreFloats() {
		return [Node([Node(cast(real) 0.0), Node(cast(real) 1.0), Node(cast(real)-1.0), Node(real.infinity), Node(-real.infinity), Node(real.nan), Node(real.nan)])];
	}
...
unittest {
	...
	alias testSet = AliasSeq!("construct-binary", "construct-bool", "construct-custom", "construct-float", "construct-int", "construct-map", "construct-merge", "construct-null", "construct-omap",
		"construct-pairs", "construct-seq", "construct-set", "construct-str-ascii", "construct-str-utf8", "construct-str", "construct-timestamp", "construct-value", "duplicate-merge-key",
		"float-representer-2.3-bug", "invalid-single-quote-bug", "more-floats", "negative-float-bug", "single-dot-is-not-float-bug", "timestamp-bugs", "utf8");
