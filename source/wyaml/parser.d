...
package final class Parser {
	...
	/**
		 ...
		 *          false otherwise.
		 */
	public bool checkEvent(EventID[] ids...) @trusted {
		//Check if the next event is one of specified types.
	...
	/**
		 ...
		 */
	public immutable(Event) peekEvent() @trusted {
		if (currentEvent_.isNull && state_ !is null) {
			currentEvent_ = state_();
		}
		if (!currentEvent_.isNull) {
			return cast(immutable Event) currentEvent_;
		}
	...
	/**
		 ...
		 * Must not be called if there are no events left.
		 */
	public immutable(Event) getEvent() @trusted {
	...
	private Event delegate() popState() @trusted {
		enforce(states_.length > 0, new YAMLException("Parser: Need to pop state but no states left to pop"));
	...
	private Mark popMark() @trusted {
		enforce(marks_.length > 0, new YAMLException("Parser: Need to pop mark but no marks left to pop"));
	...
	private Event parseDocumentStart() @trusted {
		//Parse any extra document end indicators.
		while (scanner_.checkToken(TokenID.DocumentEnd)) {
			scanner_.getToken();
		}
		...
		if (!scanner_.checkToken(TokenID.StreamEnd)) {
			...
			enforce(scanner_.checkToken(TokenID.DocumentStart), new ParserException("Expected document start but found " ~ scanner_.peekToken().idString, scanner_.peekToken().startMark));
	...
	private Event parseDocumentContent() {
		if (scanner_.checkToken(TokenID.Directive, TokenID.DocumentStart, TokenID.DocumentEnd, TokenID.StreamEnd)) {
			state_ = popState();
	...
	private TagDirective[] processDirectives() @system {
		...
		while (scanner_.checkToken(TokenID.Directive)) {
			...
			if (token.directive == DirectiveType.YAML) {
				enforce(yamlVersion_ is null, new ParserException("Duplicate YAML directive", token.startMark));
				const minor = value.split(".")[0];
				enforce(minor == "1", new ParserException("Incompatible document (version 1.x is required)", token.startMark));
				...
				foreach (ref pair; tagDirectives_) {
					// handle
					const h = pair.handle;
					enforce(h != handle, new ParserException("Duplicate tag handle: " ~ handle, token.startMark));
				}
				tagDirectives_ ~= TagDirective(handle, cast(string) value[token.valueDivider .. $]);
			}
	...
	///Parse a node.
	private Event parseNode(const Flag!"block" block, const Flag!"indentlessSequence" indentlessSequence = No.indentlessSequence) @trusted {
		...
		if (get(TokenID.Anchor, Yes.first, anchor)) {
			get(TokenID.Tag, No.first, tag);
		} else if (get(TokenID.Tag, Yes.first, tag)) {
			get(TokenID.Anchor, No.first, anchor);
		}

		if (tag !is null) {
			tag = processTag(tag, tagHandleEnd, startMark, tagMark);
		}
		...
		if (scanner_.checkToken(TokenID.Scalar)) {
			...
			auto value = token.style == ScalarStyle.DoubleQuoted ? handleDoubleQuotedScalarEscapes(token.value) : cast(string) token.value;

			implicit = (token.style == ScalarStyle.Plain && tag is null) || tag == "!";
			bool implicit_2 = (!implicit) && tag is null;
			state_ = popState();
			return scalarEvent(startMark, token.endMark, Anchor(anchor), Tag(tag), tuple(implicit, implicit_2), value, token.style);
		}
		...
		if (anchor != null || tag !is null) {
			...
			return scalarEvent(startMark, endMark, Anchor(anchor), Tag(tag), tuple(implicit, false), "");
		}

		const token = scanner_.peekToken();
		throw new ParserException("While parsing a " ~ (block ? "block" : "flow") ~ " node", startMark, "expected node content, but found: " ~ token.idString, token.startMark);
	}

	/// Handle escape sequences in a double quoted scalar.
	///
	/// Moved here from scanner as it can't always be done in-place with slices.
	private string handleDoubleQuotedScalarEscapes(string tokenValue) {
		string notInPlace;
		bool inEscape = false;
		...
		for (string oldValue = tokenValue; !oldValue.empty();) {
			...
			if (c.among!escapeHexSeq) {
				...
				assert(!hex.canFind!(d => !d.isHexDigit), "Scanner must ensure the hex string is valid");

				bool overflow;
				const decoded = cast(dchar) parse!int(hex, 16u);
				if (notInPlace is null) {
					appender.put(decoded.text);
				} else {
					notInPlace ~= decoded;
				}
				continue;
	...
	/**
		 ...
		 */
	private string processTag(const string tag, const size_t handleEnd, const Mark startMark, const Mark tagMark) const @trusted {
		...
		if (handle.length > 0) {
			...
			enforce(replacement !is null, new ParserException("While parsing a node", startMark, "found undefined tag handle: " ~ handle, tagMark));
	...
	private Event parseBlockNode() @safe {
		return parseNode(Yes.block);
	}

	private Event parseFlowNode() @safe {
		return parseNode(No.block);
	}

	private Event parseBlockNodeOrIndentlessSequence() @safe {
		return parseNode(Yes.block, Yes.indentlessSequence);
	}

	///block_sequence ::= BLOCK-SEQUENCE-START (BLOCK-ENTRY block_node?)* BLOCK-END

	///Parse an entry of a block sequence. If first is true, this is the first entry.
	private Event parseBlockSequenceEntry(Flag!"first" first)() @trusted {
		static if (first) {
			marks_ ~= scanner_.getToken().startMark;
		}
		...
		if (!scanner_.checkToken(TokenID.BlockEnd)) {
			const token = scanner_.peekToken();
			throw new ParserException("While parsing a block collection", marks_.back, "expected block end, but found " ~ token.idString, token.startMark);
		}
	...
	private Event parseIndentlessSequenceEntry() @trusted {
		if (scanner_.checkToken(TokenID.BlockEntry)) {
			const token = scanner_.getToken();

			if (!scanner_.checkToken(TokenID.BlockEntry, TokenID.Key, TokenID.Value, TokenID.BlockEnd)) {
	...
	private Event parseBlockMappingKey(Flag!"first" first)() @trusted {
		static if (first) {
			marks_ ~= scanner_.getToken().startMark;
		}
		...
		if (!scanner_.checkToken(TokenID.BlockEnd)) {
			const token = scanner_.peekToken();
			throw new ParserException("While parsing a block mapping", marks_.back, "expected block end, but found: " ~ token.idString, token.startMark);
		}
	...
	private Event parseBlockMappingValue() @trusted {
	...
	private Event parseFlowSequenceEntry(Flag!"first" first)() @trusted {
		static if (first) {
			marks_ ~= scanner_.getToken().startMark;
		}
		...
		if (!scanner_.checkToken(TokenID.FlowSequenceEnd)) {
			static if (!first) {
				if (scanner_.checkToken(TokenID.FlowEntry)) {
					...
					const token = scanner_.peekToken();
					throw new ParserException("While parsing a flow sequence", marks_.back, "expected ',' or ']', but got: " ~ token.idString, token.startMark);
				}
			}
	...
	private Event parseFlowKey(in Event delegate() nextState) @trusted {
		const token = scanner_.getToken();

		if (!scanner_.checkToken(TokenID.Value, TokenID.FlowEntry, TokenID.FlowSequenceEnd)) {
			states_ ~= nextState;
			return parseFlowNode();
		}
	...
	private Event parseFlowSequenceEntryMappingKey() @safe {
		return parseFlowKey(&parseFlowSequenceEntryMappingValue);
	}

	///Parse a mapping value in a flow context.
	private Event parseFlowValue(TokenID checkId, in Event delegate() nextState) @trusted {
		if (scanner_.checkToken(TokenID.Value)) {
			const token = scanner_.getToken();
	...
	private Event parseFlowSequenceEntryMappingValue() @safe {
		return parseFlowValue(TokenID.FlowSequenceEnd, &parseFlowSequenceEntryMappingEnd);
	}
	...
	private Event parseFlowMappingKey(Flag!"first" first)() @trusted {
		static if (first) {
			marks_ ~= scanner_.getToken().startMark;
		}
		...
		if (!scanner_.checkToken(TokenID.FlowMappingEnd)) {
			static if (!first) {
				if (scanner_.checkToken(TokenID.FlowEntry)) {
					...
					const token = scanner_.peekToken();
					throw new ParserException("While parsing a flow mapping", marks_.back, "expected ',' or '}', but got: " ~ token.idString, token.startMark);
				}
			}
